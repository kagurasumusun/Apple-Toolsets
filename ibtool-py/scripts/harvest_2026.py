#!/usr/bin/env python3
"""harvest_2026: 新セッション 0z6pfERbIkK3HRrW097T で大量 Apple フィクスチャ取得

1 接続 = 1 巨大 batch で send-keys 制限回避。各 platform を独立接続で処理。
"""
...
REMOTE = '/tmp/ibtool_2026'
LOCAL = '/home/user/ibtool-py/fixtures/mass2'
...
def process(name, content):
    ...
    cmds = [f"cd {REMOTE}",
            f"rm -f {name}.nib {name}*.nib {name}_*.plist {name}.xlf {name}.strings {name}_gen.strings {name}_*.xib {name}_*.bin {name}_*.txt",
