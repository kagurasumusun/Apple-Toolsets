#!/usr/bin/env python3
"""harvest_2026b: 各 platform に 1 接続、1 ファイルずつ慎重に (tab補完問題回避)

ファイル名を短くし、bash 補完が発動しないようにする。
"""
...
REMOTE = '/tmp/h26'
LOCAL = '/home/user/ibtool-py/fixtures/mass3'

# 短め名: ファイル名に '-' を 1 個以下に減らす
SAMPLES = {
...
def process(name, content):
    print(f'  {name}...')
    fn = f'{name}.xib' if name != 'i' else 'storyboard'
    localp = f'/tmp/{fn}'
    ...
    # Disable tab completion (CRITICAL)
    chan.send(b"bind 'set disable-completion on' && stty -echo\n")
    time.sleep(0.5)
    while chan.recv_ready():
        chan.recv(65536)

    out_ext = '.storyboardc' if name == 'i' else '.nib'
    # Use single line per command (no newlines except between)
    # 1 chain: run all commands at once
    cmds = [f"cd {REMOTE}",
            f"rm -f {name}.nib {name}*.nib {name}.plist {name}.xlf {name}.strings {name}_*.xib {name}_*.bin {name}_*.txt",
            f"ibtool --compile {name}{out_ext} {fn}"]
    for c in ['classes', 'objects', 'hierarchy', 'connections', 'all', 'version-history',
        ...
        cmds.append(f"ibtool --{c} {fn} > {name}_{c}.plist")
    cmds.append(f"ibtool --export-xliff {name}.xlf --source-language en --target-language ja {fn}")
    cmds.append(f"ibtool --export-strings-file {name}.strings {fn}")
    cmds.append(f"ibtool --generate-strings-file {name}_gen.strings {fn}")
    cmds.append(f"ibtool --write {name}_out.xib {fn}")
    cmds.append(f"ibtool --upgrade --write {name}_up.xib {fn}")
    cmds.append(f"ibtool --remove-plugin-dependencies --write {name}_rpd.xib {fn}")
    cmds.append(f"ibtool --strip --compile {name}_strip{out_ext} {fn}")
    cmds.append(f"ibtool --all --output-format binary1 {fn} > {name}_all.bin")
    cmds.append(f"ibtool --all --output-format human-readable-text {fn} > {name}_all.txt")
    cmds.append(f'echo IBLT_{name}')

    # Use ; instead of && to continue even on error
    big = ' ; '.join(cmds)
    chan.send(big + '\n')
    M = f'IBLT_{name}'
    buf = b''
    ...
    for fn2 in sftp.listdir(REMOTE):
        if fn2.startswith('.') or not fn2.startswith(name + '.'):
            continue
        # Strip name prefix to avoid "a_a_x.plist" double naming
        suffix = fn2[len(name)+1:] if fn2.startswith(name + '.') else fn2
        local = f'{LOCAL}/{name}_{suffix}'
        try:
            sftp.get(f'{REMOTE}/{fn2}', local)
            count += 1
        except Exception:
            pass
    print(f'    {count} files')
