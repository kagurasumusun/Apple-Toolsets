#!/usr/bin/env python3
"""harvest_v10: 1 send で全操作 (1 platform 全部)

bash 起動後の send-keys 1 回しか通らないので、1 つの巨大コマンドとして全部実行。
SFTP は別チャネル (sftp は常時動作する)。
"""
...
REMOTE = '/tmp/ibtool_v10'
LOCAL = '/home/user/ibtool-py/fixtures/probe_v10'
...
def process(name, content):
    ...
    # 1 SSH connect, 1 send (huge batch), then sftp get
    ...
    # Build HUGE single command
    cmds = [
        f"cd {REMOTE}",
        f"rm -f {name}.nib {name}_*.plist {name}.xlf {name}.strings {name}_gen.strings {name}_*.xib {name}_*.bin {name}_*.txt {name}_strip.nib",
        f"ibtool --compile {name}.nib {fn}",
    ]
    ...
    cmds.append(f"ibtool --strip --compile {name}_strip.nib {fn}")
    ...
    cmds.append(f"echo IBLT_DONE_{name}")
