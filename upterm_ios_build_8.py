...
"""
        ...
        while i < len(lines):
            ...
            if not line.strip(): 
                i+=1
                continue
            if "<< 'JSON_EOF'" in line or "<< 'PLIST_EOF'" in line:
                eof_marker = "JSON_EOF" if "JSON_EOF" in line else "PLIST_EOF"
                block = line + '\r'
                i += 1
                while i < len(lines) and lines[i].strip() != eof_marker:
                    block += lines[i] + '\r'
                    i += 1
                block += eof_marker + '\r'
                shell.send(block)
