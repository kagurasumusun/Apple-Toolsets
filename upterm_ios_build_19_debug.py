...
def run_upterm_test():
    ...
    try:
        ...
        # PTY を要求すると Uptermd によって強制的に tmux へアタッチされるので、
        # ここでは shell を開かず、exec_command で直接投げる。ただし、PTYなしだと失敗するコマンドがある。
        # そこで、pty=True で exec_command を実行しつつ、出力を全部吸い取るアプローチを取る。
        cmd = """
set -e
...
"""
        ...
        # We will wrap it in a bash execution inside the tmux shell.
        shell = client.invoke_shell(term='vt100', width=200, height=50)
        time.sleep(2)
        if shell.recv_ready(): shell.recv(65535)
        
        # Write to a file, then execute to bypass TMUX escaping issues
        shell.send(f"echo '{b64}' | base64 -D > /tmp/run.sh && bash /tmp/run.sh > /tmp/run.log 2>&1 && echo 'TEST_DONE'\n")
        
        print("Waiting for script to complete on remote...")
        for _ in range(60):
            time.sleep(1)
            if shell.recv_ready():
                out = shell.recv(65535).decode('utf-8', errors='ignore')
                if "TEST_DONE" in clean_tmux_output(out):
                    break
