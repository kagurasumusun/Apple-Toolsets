...
from actool_linux.core.carwriter import AssetRendition, _fixed, _identifier
...
def _premultiplied_bgra_from_pngs(pngs: Sequence[bytes]) -> tuple[int, int, bytes]:
    ...
    from actool_linux.core.carwriter import _png_premultiplied_bgra
    width = height = None
    canvas: list[int] = []
    for png in pngs:
...
def imagestack_renditions(stack_name: str, layers: Sequence[StackLayerImage], *,
    ...
    from actool_linux.core.carwriter import make_deepmap_csi_variant  # local import to avoid cycles
