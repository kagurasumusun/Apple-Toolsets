...
def main(argv: list[str] | None = None) -> int:
    ...
    if "--compile" in args:
        ...
        if idx == len(args) - 1 or args[idx + 1].startswith("-"):
            ...
            if is_human:
                ...
                from actool_linux.tools.diagnostics import unknown_argument_plist
    ...
    if ns.version:
        if ns.output_format == "xml1":
            from actool_linux.tools.diagnostics import version_plist
            write_stdout_bytes(version_plist(short_version=ns.compatibility_xcode_version))
        else:
            print(VERSION)
        return 0
    if ns.capabilities:
        from actool_linux.tools.capabilities import capability_report
    ...
    if ns.output_format == "xml1":
        from actool_linux.tools.diagnostics import result_plist
        ...
        from actool_linux.tools.diagnostics import render_human_readable
        include_results = bool(resolved_inputs) and any(path.exists() for path in resolved_inputs) and not (ns.app_icon and ns.output_partial_info_plist is None)
