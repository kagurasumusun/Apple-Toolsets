...
from actool_linux.codecs import lzfse_compat
from actool_linux.codecs import dmp2mini
...
from actool_linux.core.coreui import CoreUIProfile, resolve_profile
from actool_linux.codecs.paletteimg import build_palette_img_wrapper
...
from actool_linux.assets.texture import TextureAuxiliaryFlag, TextureReference, build_texture_auxiliary_flag, build_texture_reference_payload
...
def _dmp2_lzfse_stream(width: int, height: int, raw: bytes, bpp: int, version: int) -> bytes:
    ...
    if _OPTIMIZE_MODE in ("omega+", "nexus", "alpha", "omni"):
        from actool_linux.codecs.omega_plus import optimize_dmp2_payload
...
def make_deepmap_csi_variant(data: bytes, filename: str, *, scale: int = 1,
    ...
    if use_cbck and _OPTIMIZE_MODE in ("smart", "hybrid", "alpha", "omni", "omega"):
        ...
        from actool_linux import _csi_png_cbck as _optimized_cbck
...
def _csi_png_cbck(data: bytes, filename: str, *, scale: int = 1) -> bytes:
    ...
    if _OPTIMIZE_MODE == "smart":
        from actool_linux.codecs.smart_cbck import smart_encode_png_cbck
