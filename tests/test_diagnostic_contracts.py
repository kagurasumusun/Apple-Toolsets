...
class DiagnosticContractTests(unittest.TestCase):
 def test_malformed_is_notice_with_empty_results(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=compile_catalogs([c],CompileOptions(Path(td)/'out',platform='iphoneos',minimum_deployment_target='15.0'))
 ...
 def test_duplicate_slot_compiles_deterministically(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=compile_catalogs([c],CompileOptions(Path(td)/'out',platform='iphoneos',minimum_deployment_target='15.0'))
   self.assertTrue(r.ok);self.assertTrue((Path(td)/'out'/'Assets.car').is_file())
 ...
 def test_missing_requested_appicon_is_deferred_error(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=compile_catalogs([c],CompileOptions(Path(td)/'out',platform='iphoneos',minimum_deployment_target='15.0',app_icon='NoSuchIcon',partial_info_plist=partial))
 ...
 def test_missing_icon_and_launch_are_ordered_errors(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=compile_catalogs([c],CompileOptions(Path(td)/'out',platform='iphoneos',minimum_deployment_target='15.0',app_icon='NoIcon',launch_image='NoLaunch',partial_info_plist=partial))
