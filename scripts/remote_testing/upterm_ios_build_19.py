...
def run_upterm_test():
    ...
    try:
        ...
        # PTY なし・直接コマンドを送って実行するが、ssh の環境だと .bash_profile 等が呼ばれないので
        # 明示的に python や zip コマンドのパスが通るように PATH を指定するか、フルパスにする。
        cmd = """
bash -l -c '
set -e
...
"""
        ...
        # So we pipe the base64 command into bash.
