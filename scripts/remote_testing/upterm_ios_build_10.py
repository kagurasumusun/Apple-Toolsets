...
def run_upterm_test():
    ...
    try:
        ...
        # Build script on Mac side to avoid tmux interactive escaping issues
        cmd = """cat << 'INNER_EOF' > /tmp/build_ipa.sh
#!/bin/bash
set -e
...
"""
        ...
        while i < len(lines):
            ...
            if "cat <<" in line:
                ...
                block = line + '\r'
                i += 1
                while i < len(lines) and eof_marker not in lines[i]:
                    block += lines[i] + '\r'
                    i += 1
                block += eof_marker + '\r'
                shell.send(block)
