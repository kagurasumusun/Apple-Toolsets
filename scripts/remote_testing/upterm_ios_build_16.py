...
def run_upterm_test():
    ...
    try:
        ...
        # Upterm forces tmux attach if we use PTY, so we use invoke_shell and script encoding to bypass UI rendering bugs
...
"""
        ...
        # 1行で投げるが、パイプを使ってtmuxプロンプトに流す
        cmd = f"echo {b64} | base64 -D > /tmp/build_ipa.sh && bash /tmp/build_ipa.sh && echo 'DONE_IPA'\r"
        shell.send(cmd)
