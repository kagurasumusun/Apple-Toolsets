...
"""
        ...
        while i < len(cmd_lines):
            ...
            if "cat <<" in line:
                eof_marker = line.split("'")[1]
                block = line + '\n'
                i += 1
                while i < len(cmd_lines) and eof_marker not in cmd_lines[i]:
                    block += cmd_lines[i] + '\n'
                    i += 1
                block += eof_marker + '\n'
                shell.send(block.replace('\n', '\r'))
                time.sleep(2)
            else:
                shell.send(line + '\r')
                time.sleep(1)
        ...
        for _ in range(30):
            ...
            if shell.recv_ready():
                ...
                if "DONE_BUILD" in clean_tmux_output(out):
                    break
        ...
        try:
            ...
            # Try to get the raw compile log
            shell.send("cat /Users/runner/work/build_ios_app/ios-dev/testapp/swift-test-build/build_final.log\r")
            time.sleep(2)
            if shell.recv_ready():
                print(clean_tmux_output(shell.recv(65535).decode('utf-8', errors='ignore')))
