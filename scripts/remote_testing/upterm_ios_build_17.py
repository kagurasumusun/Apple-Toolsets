...
def run_upterm_test():
    ...
    try:
        ...
        print("Executing iOS Build Pipeline on Remote Mac via exec_command...")
...
"""
        ...
        # PTYを使用せずに、直接コマンドを渡す。ただし ssh で uptermd に繋ぐと
        # 必ず force_command の tmux attach が走るため、exec_command でも tmux が絡むかもしれない。
        # upterm の仕様次第だが、試してみる。
        stdin, stdout, stderr = client.exec_command(f"echo '{b64}' | base64 -D > /tmp/build_ipa.sh && bash /tmp/build_ipa.sh")
        
        # We need to read everything until it closes
        while not stdout.channel.exit_status_ready():
            ...
            if stderr.channel.recv_stderr_ready():
                ...
                print(clean_tmux_output(err), end="")
            time.sleep(0.5)
            
        # Flush the rest
        if stdout.channel.recv_ready():
            print(clean_tmux_output(stdout.read().decode('utf-8', errors='ignore')))
        if stderr.channel.recv_stderr_ready():
            print(clean_tmux_output(stderr.read().decode('utf-8', errors='ignore')))
