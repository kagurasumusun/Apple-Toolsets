...
def run_upterm_test():
    ...
    try:
        ...
        # 1. 巨大スクリプトをcat等で送り込むのはtmux環境で文字欠けやパニックを起こすので、
        # pythonでスクリプトの文字列を作成し、ワンライナーで base64 エンコードしたものをMac側でデコードさせる
...
"""
        ...
        cmd = f"echo '{b64_script}' | base64 --decode > /tmp/build_ipa.sh && bash /tmp/build_ipa.sh && echo DONE_BUILD\r"
        shell.send(cmd)
